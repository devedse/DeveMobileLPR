...
The detector's trained YOLO graph is shared, but its execution adapter is platform-specific. Windows keeps the end-to-end ONNX graph and attempts DirectML before CPU. The production Android build removes the ONNX Non-Maximum Suppression tail at build time, converts the fixed raw box/score graph to float32 LiteRT, and executes it in NHWC layout. The generated raw graph also replaces the detector head's uneven 65-channel Split with equivalent static Slice nodes and removes a singleton ReduceMax; this preserves outputs exactly while allowing NNAPI-compatible graph partitioning. Android first creates a GPU-only LiteRT compiled model and accepts it only after a complete warm inference; if that fails, it creates an explicit CPU model. CI also packages an ONNX-Raw comparison APK from the same pre-NMS graph. That variant executes the NCHW graph through ONNX Runtime but deliberately uses the same confidence filtering, coordinate mapping, C# NMS, OCR, and tracking as LiteRT, isolating the runtime as the comparison variable. Production releases remain LiteRT-only.

OCR remains on ONNX Runtime. Android creates NNAPI and XNNPACK OCR candidates, warms both with the real tensor shape, measures two hot executions, and retains only the faster session; CPU is the final fallback. Windows attempts DirectML first. Inference sessions and reusable input buffers are long-lived, and runs are serialized per model. The selected detector and OCR backend names are retained on every recognition diagnostic frame and shown in Drive and Analyze debug diagnostics.

The Android Release project currently keeps Mono AOT disabled because the pinned .NET 10 Android toolchain's cross-assembler fails both universal and arm64 profiled-AOT packaging. Model execution is unaffected because ONNX Runtime providers are native. The managed preprocessing optimizations are therefore explicit algorithm/allocation improvements rather than depending on an unshippable build flag.
