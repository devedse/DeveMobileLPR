...
The shared-XAML, virtual-view, handler, native-host, and device-resource boundaries are documented
in [.NET MAUI native-control architecture](maui-native-control-architecture.md).

## Platform boundary

`DeveMobileLPR.Application` is a plain `net10.0` project: it references Core and Inference, but not MAUI, Android, iOS, WinUI, or Storage. It owns the complete Drive and Analyze workflows, their snapshots and diagnostics, recognition-session lifetime, frame backpressure, overlay visibility/projection, and the ports through which a host supplies storage, settings, model creation, video input/decoding, location, dispatch, and device feedback.

The MAUI project is the composition root. Android, iOS, and Windows register different implementations of `IRecognitionPipelineProvider`, `IDriveVideoInput`, and `IVideoFileBackend`; Android and iOS also supply native location trackers. ViewModels and shared workflows never select a platform or construct a native implementation. A target without a capability registers a shared `Unsupported*` or passive policy instead of placing a fake platform implementation under that target. Both Drive and Analyze receive the same platform pipeline provider and the same `RecognitionTuningConfiguration`, so model verification/installation and recognition-pipeline creation cannot drift between modes. iOS uses AVFoundation for camera, LL-HLS, and recorded-video access; its `CVPixelBuffer` NV12 planes are copied through the shared stride-aware planar frame factory.
...
| --- | --- | --- | --- | --- |
| Background scanning | `IBackgroundScanningManager`, `UnsupportedBackgroundScanningManager` | `Platforms/Android/Background` | None | Android owns its foreground service; iOS uses a no-op implementation and unsupported targets use the shared fallback. |
...
| Inference execution | Recognition pipeline contracts and shared preprocessing/postprocessing | `Platforms/Android/Inference`, `Platforms/iOS` | `Platforms/Windows/Inference` | Android and iPhone use LiteRT GPU/Metal adapters; Windows uses ONNX Runtime/DirectML. |
...
`DriveInputConfigurationPlanner` is the shared boundary between saved/UI input configuration and native adapters. It resolves available capabilities, normalizes resolutions, zoom, duplicate or unavailable profiles, and network URLs without imposing a device camera-count policy. Android and Windows consume that plan and retain only native activation and preview responsibilities. Windows device enumeration and capability mapping have one source of truth in `WindowsDriveSourceCatalog`; the webcam adapter receives those discovered choices instead of enumerating them again.

Camera preview, AI orientation, and detection projection changes must follow the explicit contracts and Pixel 9 Pro acceptance matrix in [camera-preview-and-detection-geometry-plan.md](camera-preview-and-detection-geometry-plan.md). The MAUI/native ownership pattern is documented separately in [.NET MAUI native-control architecture](maui-native-control-architecture.md). Detection rendering is a single cross-platform `DriveOverlayView` (a MAUI `GraphicsView`) that the live drive view and analysis review both use, so a detection cannot look one way on Android, another on Windows, and a third in review. `DriveOverlayFactory` maps live recognition and offline analysis onto the same `DriveOverlay` list, `DriveOverlayLayout` does the debug filtering and fit/fill projection, `ConfirmedOverlayTracker` owns how long a confirmed plate lingers once its car leaves view, and `DriveOverlayStyle` holds one entry per overlay kind. `DrivePreviewPresenter.xaml` gives `CameraPreview` and `DriveOverlayView` exactly the same layout rectangle. The handler reports the actual fit/fill mode through the read-only `CameraPreview.ScaleMode` property, while the Android input adapter reports normalized per-source panels through the read-only `SourceViewports` property; the overlay binds to both. Android forces `PreviewView.ImplementationMode.Compatible` so the preview is a `TextureView` composited in normal view order rather than a `SurfaceView` hole. ARGB and BGRA decoded frames use shared Core factories and one YUV color conversion formula. Windows locks `SoftwareBitmap` memory directly and passes its actual stride to the shared BGRA factory, avoiding the previous full-frame managed byte-array allocation on every webcam frame.
...
Android physical multi-camera capture first attempts one YUV `ImageReader` and one native preview surface per selected lens. Some devices accept that four-output session but cannot sustain it, so startup explicitly falls back to two YUV outputs when Camera2 rejects the native-preview configuration. In fallback mode, `PhysicalYuvPreviewView` downsamples a throttled preview from the same pooled YUV copy passed to recognition. Each physical source has its own ImageReader callback thread, while preview and recognition remain latest-only backpressure boundaries in both modes.
...
The source detector input is `[1,3,608,608]` RGB `float32`; mobile LiteRT exposes the equivalent `[1,608,608,3]` layout. Both are normalized to 0–1 and letterboxed with value 114. Windows decodes end-to-end rows as `[batch,x1,y1,x2,y2,class,score]`. Android and iPhone receive raw `[1,7581,4]` boxes and `[1,7581,1]` scores, then apply the shared confidence filter and NMS. Both paths map final coordinates back through letterbox padding and the road ROI.
...
The detector's trained YOLO graph is shared, but its execution adapter is platform-specific. Windows keeps the end-to-end ONNX graph and attempts DirectML before CPU. The mobile build removes the ONNX Non-Maximum Suppression tail at build time, converts the fixed raw box/score graph to float32 LiteRT, and executes it in NHWC layout. The generated raw graph also replaces the detector head's uneven 65-channel Split with equivalent static Slice nodes and removes a singleton ReduceMax while preserving the fixed outputs. Shared C# code owns confidence filtering, coordinate mapping, and NMS.

Android and iPhone use the same float32 LiteRT CCT-S V2 graph while preserving its uint8 NHWC input and two float outputs. Android requests the LiteRT GPU accelerator; iPhone attaches the LiteRT Metal delegate. Detector and OCR accept acceleration only after tensor-contract validation and a complete warm inference; if that fails, they create an explicit CPU model. Windows keeps both models on ONNX Runtime and attempts DirectML before CPU. Preprocessing, OCR decoding, and detector postprocessing live below the platform runners so every runtime uses identical recognition semantics. Model sessions and reusable buffers are long-lived. Multi-source capture has an independent latest-frame slot and stream processor per source, while access to a shared detector/OCR pipeline is serialized. The selected detector and OCR backend names are retained on every recognition diagnostic frame and shown in Drive and Analyze diagnostics.
...
The Analyze tab is intentionally separate from Drive and History. Android and iPhone copy a selected document-provider video into durable private app storage because those streams cannot be reopened reliably by their platform media decoders. Windows reuses the selected local path when available and falls back to a private staged copy. A source is removed only when no saved analysis references it. Detections and confirmations are persisted as compact JSON metadata, atomically replaced, and remain separate from sighting history. Raw frames, previews, and plate crops are never persisted.

Android's media retriever, iPhone's AVAsset image generator, and a Windows Media Foundation Source Reader implement the shared `IVideoFrameSource` contract. These adapters own only native media opening, timeline metadata, decoding, and conversion into pooled planar YUV. Android and iPhone use timestamp-based retrieval. Windows requests NV12 and reads forward sequentially, discarding unsampled frames without seeking; it permits hardware transforms and DXVA while retaining system-memory output so driver-specific GPU-surface failures cannot abort an analysis. Recognition receives the decoded source resolution on every platform: the detector still preprocesses to 608×608, while OCR crops from the full source frame. Only lazy review thumbnails are scaled to 1280 pixels wide.

The platform-independent `VideoAnalysisEngine` owns sampling, duration limits, cancellation, progress, serialized runs, and compact result projection. It delegates recognition, geometry resets, tracking, and temporal consensus to `RecognitionStreamProcessor`, the same stateful component used by Drive mode. Shared analysis records, `VideoFrameSampling`, and `VideoFrameTimeline` keep processing and persistence semantics consistent across Android, iPhone, and Windows. This boundary keeps native media APIs out of the recognition workflow while ensuring live and recorded frames exercise the same recognition behavior.
