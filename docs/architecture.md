...
Android physical multi-camera capture deliberately exposes only two Camera2 outputs: one YUV `ImageReader` per selected lens. The previous pair of native preview surfaces plus pair of 4K YUV readers configured successfully on the Pixel 9 Pro but could not run sustainably; one lens eventually stopped producing both preview and analysis frames. `PhysicalYuvPreviewView` now downsamples a throttled preview from the same pooled YUV copy passed to recognition, so preview and detection share orientation, aspect, crop, and frame provenance without consuming extra camera outputs. Each physical source has its own ImageReader callback thread, while preview and recognition both remain latest-only backpressure boundaries.
