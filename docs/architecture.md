...
Inference sessions and input `OrtValue` objects are long-lived. Android creates NNAPI and XNNPACK candidates for each model, warms both with the model's real tensor shape, measures two hot executions, and retains only the faster session. NNAPI disables its slow reference-CPU device so unsupported nodes fall back to optimized ONNX Runtime kernels; detector NNAPI may relax to FP16, while OCR remains FP32 by default to protect character probabilities. If a candidate cannot initialize or execute, it is discarded and the next provider is tried; the ordinary CPU provider is the final fallback. Provider names and warm timings are emitted through the existing initialization diagnostics. Windows attempts DirectML first, allowing supported graph nodes to run on the GPU. Runs are serialized per model because their input buffers are reused.
