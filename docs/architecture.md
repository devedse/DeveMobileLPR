...
USB Video Class (UVC) cameras use the operating-system camera stack rather than a bundled vendor driver. Windows discovers them through `DeviceClass.VideoCapture` and captures them with `MediaCapture`. Android discovers an external camera through CameraX when the device firmware provides the optional external Camera HAL; such cameras appear as **USB/UVC camera** in the drive input picker. Android devices without an external Camera HAL cannot expose a USB camera to CameraX and require vendor firmware support.
